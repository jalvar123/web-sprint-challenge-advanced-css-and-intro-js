...
function get20s(arr, years){
  ...
  for(let i = 0;i<arr.length;i++){
    if(arr[i].years.includes(years)){
      twenT.push(arr[i].name);
    }
  }
  return twenT;
}
...
function addArtist(ids,names,yearss, genres, nationalities, bios){

    let nuArtist =[{id: ids, name: names, years:yearss, genre:genres, nationality:nationalities, bio:bios}];
    artists.push(...nuArtist);
    return artists;
 }
 console.log(addArtist(23, "Jesus", "1997-Friday", "Web Design", "All American", "Lorem ipsum dolor sit amet, consectetur adipiscing elit. "));
...
function lotsOfArt(arty){

  let lotsArtists = [];
  for (let i=0; i<arty.length; i++){
    if (arty[i].paintings > 100){
      lotsArtists.push(arty[i])
    }
  }
  return lotsArtists;
}
